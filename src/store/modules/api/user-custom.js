...
const actions = {
    ...
    saveUsercustom ({ commit, state, dispatch }, usercustom = {}) {
        let url = `usercustom`
        url = `${url}?${AJAX_MOCK_PARAM}=cmdb&invoke=usercustom`
        return $http.post(url, usercustom, { cancelWhenRouteChange: false }).then(() => {
    ...
    searchUsercustom ({ commit, state, dispatch }, { config }) {
        ...
        let url = `usercustom/user/search`
        url = `${url}?${AJAX_MOCK_PARAM}=cmdb&invoke=search`
        return $http.post(url, {}, mergedConfig).then(usercustom => {
            commit('setUsercustom', usercustom)
            return usercustom
        })
    },
    ...
    getGlobalUsercustom ({ commit }, { config }) {
        ...
        let url = `usercustom/default/model`
        url = `${url}?${AJAX_MOCK_PARAM}=cmdb&invoke=model`
        return $http.post(url, {}, mergedConfig).then(usercustom => {
